        ...
        <div className="relative group flex items-center hover:bg-black/5 p-4 transition w-full">
            <div className="group flex gap-x-2 items-start w-full">
                ...
                <div className="flex flex-col w-full">
                    ...
                    {isImage && (
                        <a href={fileUrl as string} target="_blank" rel="noopener noreferrer" className="relative aspect-square rounded-md mt-2 overflow-hidden border flex items-center bg-secondary h-48 w-48">
                    ...
                    {isPDF && (
                        <div className="relative flex items-center p-2 mt-2 rounded-md bg-background/10">
                            ...
                            <a href={fileUrl as string} target="_blank" rel="noopener noreferrer" className="ml-2 text-sm text-indigo-500 dark:text-indigo-400 hover:underline">{content}</a>
                        </div>
                    )}
                    {!fileUrl && !isEditing && (
                    ...
                    {!fileUrl && isEditing && (
                        <Form {...form}>
                            <form onSubmit={form.handleSubmit(onSubmit)} className="flex items-center w-full gap-x-2 pt-2">
                                ...
                                <Button disabled={isLoading} size="sm" variant="primary">Kaydet</Button>
                            </form>
                            <span className="text-[10px] mt-1 text-zinc-400">İptal için ESC, Kaydetmek için Enter</span>
                        </Form>
                    )}
                </div>
            </div>
